...
interactionRegistry.register('chalkboard', (scene: Scene) => {
    ...
    const border = scene.add.rectangle(centerX, centerY, interfaceWidth + 16, interfaceHeight + 16, 0x8b4513)
    border.setDepth(3000)

    const chalkboardBg = scene.add.rectangle(centerX, centerY, interfaceWidth, interfaceHeight, 0x2d5016)
    chalkboardBg.setDepth(3001)

    const chalkboardText = scene.add.text(centerX, centerY - 30, 'Welcome to the platform', {
        fontSize: '48px',
        color: '#ffffff',
    ...
    const closeButton = scene.add.text(centerX, centerY + interfaceHeight / 2 - 60, 'Click anywhere, press Q or ESC to close', {
        fontSize: '24px',
        color: '#cccccc',
        fontFamily: 'Arial, sans-serif',
        align: 'center'
    ...
    const elements = [overlay, border, chalkboardBg, chalkboardText, closeButton]
